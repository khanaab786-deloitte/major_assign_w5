with
mutual_fund_first as (select code, sum(nav) as sum_nav_first from "MAJOR_ASSIGN"."MAJOR_SCHEMA"."NAV_HISTORY" where nav_date = '2018-12-01'
group by code),
...
select name,calculate_performance.cal from "MAJOR_ASSIGN"."MAJOR_SCHEMA"."MUTUAL_FUNDS" as mutual_fund inner join calculate_performance on mutual_fund.code = calculate_performance.cal_code
order by calculate_performance.cal desc limit 3